...
function dcn_dyn(du, u, I, t)

	du[1] = (I - g_Naf * u[2]^3 * u[3] * (u[1] - E_Na) * area_soma - 
					...
					g_Sk * u[12] * (u[1] - E_K) * area_soma -
					(u[1] - E_m) / (R_M / area_soma)) / (Cm * area_soma);
...
function dcn_dyn_noCa(du, u, I, t)
...
area_soma = d_soma * d_soma * pi ;	# m^2
vol_soma = pi / 3.0 * (3.0 * d_soma * d_soma * thick_soma - 
					   6.0 * d_soma * thick_soma * thick_soma + 
					   4.0 * thick_soma * thick_soma * thick_soma) ;	# m^3
...
prob = ODEProblem(dcn_dyn, u0, tspan, I) ;
