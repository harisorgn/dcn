...
function initialise_compartments(comp_sym::Array{Symbol}, diam_v::Array{Float64}, len_v::Array{Float64}, C::Array{Int64})
	...
	for i = 1 : length(comp_sym)
		...
		if comp_sym[i] == :s
			...
			comp_v[i] = soma_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_idx[find(C[i,:])], n_eq_s, 1.0) ;

		elseif comp_sym[i] == :axhill

			comp_v[i] = axon_hill_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_idx[find(C[i,:])], n_eq_ax, 1.0) ;

		elseif comp_sym[i] == :axis

			comp_v[i] = axon_is_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_idx[find(C[i,:])], n_eq_ax, 1.0) ;

		elseif comp_sym[i] == :pdend

			comp_v[i] = prox_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_idx[find(C[i,:])], n_eq_pd, 1.0) ;

		elseif comp_sym[i] == :ddend

			comp_v[i] = dist_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_idx[find(C[i,:])], n_eq_dd, 1.0) ;
	...
	for i = 1 : length(reduced_comp_v)
		...
		for j = 1 : length(reduced_comp_v)
			if reduced_comp_v[j].connect_to == reduced_comp_v[i].id
				push!(Ra_connect_v, reduced_comp_v[j].Ra) ;
				#push!(Ra_connect_v, reduced_comp_v[j].Ra / reduced_comp_v[j].n_comp) ;
				push!(V_connect_idx, V_idx[j]) ;
			elseif reduced_comp_v[j].id == reduced_comp_v[i].connect_to
				push!(Ra_connect_v, reduced_comp_v[i].Ra) ;
				#push!(Ra_connect_v, reduced_comp_v[i].Ra / reduced_comp_v[i].n_comp) ;
				push!(V_connect_idx, V_idx[j]) ;
			end
		end
		...
		if reduced_comp_v[i].ctype == :s
			...
			#					reduced_comp_v[i].area_sum / (pi * reduced_comp_v[i].d^2.0)) ;
			comp_v[i] = soma_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_s, 1.0) ;
		elseif reduced_comp_v[i].ctype == :axhill

			#comp_v[i] = axon_hill_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_ax,
			#						reduced_comp_v[i].area_sum / (pi * reduced_comp_v[i].l * reduced_comp_v[i].d)) ;
			comp_v[i] = axon_hill_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_ax, 1.0) ;

		elseif reduced_comp_v[i].ctype == :axis

			#comp_v[i] = axon_is_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_ax,
			#					reduced_comp_v[i].area_sum / (pi * reduced_comp_v[i].l * reduced_comp_v[i].d)) ;
			comp_v[i] = axon_is_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_ax, 1.0) ;

		elseif reduced_comp_v[i].ctype == :pdend

			#comp_v[i] = prox_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_pd,
			#						reduced_comp_v[i].area_sum / (pi * reduced_comp_v[i].l * reduced_comp_v[i].d)) ;
			comp_v[i] = prox_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_pd, 1.0) ;

		elseif reduced_comp_v[i].ctype == :ddend

			#comp_v[i] = dist_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_dd,
			#						reduced_comp_v[i].area_sum / (pi * reduced_comp_v[i].l * reduced_comp_v[i].d)) ;
			comp_v[i] = dist_dend_t(area_v[i], Ca_vol_v[i], Ra_connect_v, V_connect_idx, n_eq_dd, 1.0) ;
...
function compartment_dynamics(comp::soma_t, u::Array{Float64}, V_connect::Array{Float64}, Ie::Float64)
	...
	dv = (Ie - g_Naf_s * u[2]^3 * u[3] * (u[1] - E_Na) * comp.area - 
...
function compartment_dynamics(comp::axon_hill_t, u::Array{Float64}, V_connect::Array{Float64}, Ie::Float64)
	...
	dv = (  - g_Naf_ax * u[2]^3 * u[3] * (u[1] - E_Na) * comp.area - 
...
function compartment_dynamics(comp::prox_dend_t, u::Array{Float64}, V_connect::Array{Float64}, Ie::Float64)
	...
	dv = (  - g_Naf_pd * u[2]^3 * u[3] * (u[1] - E_Na) * comp.area - 
...
function compartment_dynamics(comp::dist_dend_t, u::Array{Float64}, V_connect::Array{Float64}, Ie::Float64)
	...
	dv = (  - g_h_dd * u[2]^2 * (u[1] - E_h) * comp.area - 
